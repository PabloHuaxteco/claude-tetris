...
- **Records locales**: persistidos en `localStorage` bajo la clave `tetris-records` como `{ scores: [{name, score, lines, level, date}], bestCombo, maxLines }` (top `MAX_RECORDS` = 5). `combo`/`maxCombo` cuentan bloqueos consecutivos que limpian ≥1 línea (se reinicia el combo al bloquear sin limpiar). `renderRecords(container, highlightIndex, provisional)` pinta la tabla (con fila provisional resaltada si la puntuación entra al top). La pantalla de inicio (`#start-overlay`) sustituye al arranque automático: `init()` sólo corre al pulsar «Jugar». En game over, si `qualifiesForTop(score)` se muestra `#name-entry` para capturar el nombre; `commitScore()` guarda vía `addScore`. Botones «Resetear records» en ambos overlays llaman a `resetRecords()`.
